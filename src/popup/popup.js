...
function windowGetSelectionString() {
...
$(document).ready(() => {
  ...
  getOptionsAndTab().then((info) => {
    ...
    // Setup user name
    const username = Utils.getUsernameFromAuthToken(
      options[Constants.OPTIONS_AUTH_TOKEN]
    );
    ...
    getTagsAndBookmark(tab.url).then((tagsAndBookmarkInfo) => {
      ...
      if (bookmark === null) {
        ...
        $readLaterCheckBox.prop(
          "checked",
          options[Constants.OPTIONS_READ_LATER]
        );

        // get selected text from page and insert to description field
        getSelectedText().then((selection) => {
          ...
          if (options[Constants.OPTIONS_QUICK_ADD]) {
            ...
            chrome.runtime.sendMessage(
              ...
              () => {
                ...
                const h = `https://pinboard.in/search/u:${username}?query=${encodeURIComponent(
                  tab.url
                )}`;
                $status.html(
                  `<a href="${h}" target="_blank">bookmarked ${t}</a>`
                );
              }
            );
          }
